...
public class BXMLSerializer implements Serializer<Object>, Resolvable {
                ...
                while (xmlStreamReader.hasNext()) {
                    ...
                    switch (event) {
                        ...
                        case XMLStreamConstants.START_ELEMENT: {
                        	if (isEmitMode()) codeEmitter.start_element(); // [nn]
                            processStartElement();
                            break;
                        }
                        ...
                        case XMLStreamConstants.END_ELEMENT: {
                            processEndElement();
                            if (isEmitMode()) codeEmitter.end_element(); // [nn]
                            break;
                        }
                    }
                }
    ...
    private void processStartElement() throws IOException, SerializationException {
            ...
            if (Character.isUpperCase(localName.charAt(0))) {
                ...
                if (i != -1
                    ...
                    try {
                        ...
                        //codeGenerator.code_new_java_instance(type, value);
                    } catch (ClassNotFoundException exception) {
                        throw new SerializationException(exception);
                ...
                if (element.value instanceof Dictionary<?, ?>) {
                    ...
                    if (beanAdapter.isReadOnly(localName)) {
                        ...
                        if (ListenerList.class.isAssignableFrom(propertyType)) {
                            ...
                            // [nn]
                            if (isEmitMode()) codeEmitter.code_decl_default_value(beanAdapter.getBean(), localName, value);
                        }
                    } else {
                        elementType = Element.Type.WRITABLE_PROPERTY;
                    }
        ...
        if (elementType == Element.Type.INCLUDE) {
            ...
            try {
                ...
                // [nn] the value should be already registered by the serializer.
            } finally {
                inputStream.close();
            }
            ...
            // [nn] value must come from outside..
        }
        ...
        switch (element.type) {
                    ...
                    if (attribute.propertyClass == null) {
                        ...
                        if (attribute.propertyClass.isInterface()) {
                            ...
                            try {
                                ...
                                // [nn]
                                System.out.println(">> [processEndElement] listenerList: "+element.value);
                            } catch (InvocationTargetException exception) {
                                throw new SerializationException(exception);
                            } catch (IllegalAccessException exception) {
                                throw new SerializationException(exception);
                            ...
                            try {
                                ...
                                // [nn]
                                System.out.println(">> [processEndElement: REFERENCE] addMethod: listener: "+listener);
                            } catch (IllegalAccessException exception) {
                                throw new SerializationException(exception);
                            } catch (InvocationTargetException exception) {
                                throw new SerializationException(exception);
                    ...
                    if (element.parent.type == Element.Type.WRITABLE_PROPERTY) {
                        ...
                        if (defaultProperty == null) {
                            if (element.parent.value instanceof Sequence<?>) {
                                ...
                                // [nn]
                                if (isEmitMode()) codeEmitter.code_add_element(sequence, element.value);
                            } else {
                            ...
                            // [nn]
                            if (isEmitMode()) {
                            	codeEmitter.code_decl_default_value(beanAdapter.getBean(), defaultPropertyName, defaultPropertyValue);
                            }
                            ...
                            if (defaultPropertyValue instanceof Sequence<?>) {
                                ...
                                // [nn]??
                                if (isEmitMode()) codeEmitter.code_set_element(beanAdapter.getBean(), defaultPropertyName, element.value);
                            }
                        }
                    }
                }
            ...
            case READ_ONLY_PROPERTY: {
                ...
                for (Attribute attribute : element.attributes) {
                    ...
                    dictionary.put(attribute.name, attribute.value);
                    // [nn]??
                    if (isEmitMode()) codeEmitter.code_set_attr(dictionary, attribute.name, attribute.propertyClass, attribute.value);
                }
            ...
            case WRITABLE_PROPERTY: {
                if (element.propertyClass == null) {
                    ...
                    // [nn]??
                    if (isEmitMode()) codeEmitter.code_set_element(dictionary, element.name, element.value);
                } else {
                    if (element.parent == null) {
    ...
    protected Serializer<?> newIncludeSerializer(Class<? extends Serializer<?>> type)
    	...
    	// [nn]
    	if (isEmitMode()) codeEmitter.code_new(type, obj);
    	return obj;
    }
